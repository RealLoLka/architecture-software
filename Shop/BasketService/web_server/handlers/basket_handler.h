...
public:
                       ...
                       HTTPServerResponse &response)
    {
        ...
        try
        {
            if (hasSubstr(request.getURI(), "/add_product") &&
                ...
                form.has("service_id"))
            {
                ...
                database::Basket result = database::Basket::add_product_to_basket(service_id,user_id);
            ...
            else if (hasSubstr(request.getURI(), "/get_basket") &&
                    (request.getMethod() == Poco::Net::HTTPRequest::HTTP_GET) && 
                    form.has("user_id"))
            {
                long user_id = atol(form.get("user_id").c_str());
                auto results = database::Basket::get_basket(user_id);
                Poco::JSON::Array arr;
                for (auto s : results)
                    arr.add(s.toJSON());
                response.setStatus(Poco::Net::HTTPResponse::HTTP_OK);
        ...
        root->set("instance", "/BasketService");
        std::ostream &ostr = response.send();
