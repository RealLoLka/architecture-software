#ifndef BASKETHANDLER_H
#define BASKETHANDLER_H
...
public:
                       ...
                       HTTPServerResponse &response)
    {
        ...
        try
        {
            ...
            if (hasSubstr(request.getURI(), "/getbasket") && (request.getMethod() == Poco::Net::HTTPRequest::HTTP_GET))
            { 
                std::cout << "search started \n";
                long user_id = atol(form.get("user_id").c_str());

                auto results = database::Basket::read_by_user_id(user_id);
                Poco::JSON::Array arr;
                for (auto s : results)
                    arr.add(s.toJSON());
